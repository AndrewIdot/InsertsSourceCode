...
class StoryMenuState extends MusicBeatState
{
	...
	var bgSprite:FlxSprite; // its still kept here so that the week offset isnt fucked

	var menuBg:FlxSprite;
	var menuBgblack:FlxSprite;
	...
	var tracksSprite:FlxSprite; // I had to fucking move the tracksSprite over here because I cant hide it
	...
	override function create()
	{
		...
		txtWeekTitle = new FlxText(FlxG.width * 0.7, 10, 0, "", 45);
		txtWeekTitle.setFormat("VCR OSD Mono", 45, FlxColor.WHITE, CENTER);

		menuBg = new FlxSprite(0, 1).loadGraphic(Paths.image('menuBgStoryMode')); //WORK YOU FUCKING DISGRACE TO HUMANITY
		menuBg.screenCenter(X);
		add(menuBg);

		menuBgblack = new FlxSprite(0, 1).loadGraphic(Paths.image('menuBgStoryModeBlack')); //here so that the tutorial text doesnt overlap
		menuBgblack.screenCenter(X);		
		...
		leftArrow = new FlxSprite(grpWeekText.members[0].x + grpWeekText.members[0].width - 930, grpWeekText.members[0].y + 125);
		...
		txtTracklist = new FlxText(FlxG.width * 0.05, tracksSprite.y + 75, 0, "", 32);
	...
	override function update(elapsed:Float)
	{
		...
		if (!movedBack && !selectedWeek)
		{
			...
			else if (controls.ACCEPT)
			{
				...
				// hiding shit so that fab's very cool confirm concept works
				// although this is pretty wasteful and can definitely be improved but im lazy and i barely know what im even doing
				
				menuBg.visible = false;
				menuBgblack.visible = false;
				...
				FlxG.sound.play(Paths.sound('confirmMenuWeek')); // reminder to self: dont put it in the shared sounds folder next time
			}
		}
